...
contract("Token Contract", function(accounts){
    ...
    it("Positive Test - Admin can mint to tokens self", async()=> {
    ...
    it("Negative Test - Minter cannot mint tokens to self", async()=> {
        try {
            await ofin.mint(minter, web3.utils.toWei("1", "ether"), {from: minter});
            assert.equal(true, false, "Minter can mint tokens to self");
        } catch (error) {
            assert.include(error.toString(), "must have admin role to mint to self", error.message);
        }
    });
